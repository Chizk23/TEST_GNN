...
export function generateTask5Mock(numNodes = 30, numEpochs = 80) {
  const links = []
  const linkSet = new Set()
  const degrees = new Array(numNodes).fill(0)
  const communities = Array.from({ length: numNodes }, (_, i) => i % 3)
  ...
  const startPos = Array.from({ length: numNodes }, (_, i) => [
    (seededRand(i * 41) - 0.5) * 6, (seededRand(i * 73) - 0.5) * 6,
  ])
  ...
  for (let epoch = 0; epoch < numEpochs; epoch++) {
    ...
    const embeddings = Array.from({ length: numNodes }, (_, i) => {
      const cx = communities[i] * 5
      const jx = (seededRand(i * 229 + epoch) - 0.5) * (0.5 * (1 - progress))
      const jy = (seededRand(i * 317 + epoch) - 0.5) * (0.5 * (1 - progress))
      return [lerp(startPos[i][0], cx, progress) + jx, lerp(startPos[i][1], 0, progress) + jy]
    })
    const lossNoise = (seededRand(epoch * 37) - 0.5) * 0.01
    const trainLoss = Math.max(0.02, 0.8 * Math.exp(-epoch / 20) + 0.05 + lossNoise)
    // t-SNE projection (slightly different from PCA — add rotation + different noise)
    const tsne = Array.from({ length: numNodes }, (_, i) => {
      const angle = communities[i] * (2 * Math.PI / 3) + Math.PI / 6
    ...
    // k-NN preservation rate (improves over epochs)
    const knnPres = Math.min(0.95, 0.2 + 0.7 * sigmoid(6 * (progress - 0.3)) + (seededRand(epoch * 89) - 0.5) * 0.02)

    // Link reconstruction AUC
    const linkAuc = Math.min(0.96, 0.5 + 0.42 * sigmoid(6 * (progress - 0.25)) + (seededRand(epoch * 97) - 0.5) * 0.015)

    // Distance matrix (compact: only first 30 nodes)
    const distMatrix = Array.from({ length: numNodes }, (_, i) =>
  ...
  const nodes = Array.from({ length: numNodes }, (_, i) => ({
    id: i, degree: degrees[i], community: communities[i],
  }))
  return { graphData: { nodes, links }, snapshots, communities }
}
